...
DEPLOY_ENV=prod
DEPLOY_HOST=ploneconf.example.com
DEPLOY_PORT=22
DEPLOY_USER=plone
...
You need either a Ubuntu or Debian based system for ploneconf.example.com, enable SSH, and install a supported version of Python 3 on that system.
...
prod:
  hosts:
    ploneconf.example.com:
      ansible_user: root
      host: ploneconf
      hostname: ploneconf.example.com
```
...
Deploy the stack defined in `devops/stacks/ploneconf.example.com.yml` to the remote server with:
