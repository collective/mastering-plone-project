...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://mastering-plone-project.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t collective/mastering-plone-project-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t collective/mastering-plone-project-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name mastering-plone-project-frontend-acceptance --link mastering-plone-project-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d collective/mastering-plone-project-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name mastering-plone-project-backend-acceptance -d collective/mastering-plone-project-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop mastering-plone-project-frontend-acceptance
	@docker stop mastering-plone-project-backend-acceptance
